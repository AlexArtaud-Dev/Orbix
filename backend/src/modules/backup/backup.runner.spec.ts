...
describe('BackupRunner', () => {
  ...
  beforeEach(async () => {
    ...
    (BackupRunner as unknown as Record<string, boolean>)['encryptedFormatRegistered'] = false;
    (BackupRunner as unknown as Record<string, boolean>)['bzip2FormatRegistered'] = false;
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: SettingsService, useValue: { get: jest.fn().mockResolvedValue({ filesRoot: '/tmp' }) } },
  ...
  describe('static format registration guards', () => {
    it('encryptedFormatRegistered initialises to false', () => {
      expect(
        (BackupRunner as unknown as Record<string, boolean>)['encryptedFormatRegistered'],
      ).toBe(false);
    });

    it('bzip2FormatRegistered initialises to false', () => {
      expect(
        (BackupRunner as unknown as Record<string, boolean>)['bzip2FormatRegistered'],
      ).toBe(false);
    });
    ...
    it('sets encryptedFormatRegistered to true after first encrypted zip creation', async () => {
      ...
      await (runner as unknown as Record<string, (...args: unknown[]) => Promise<unknown>>)[
        'createEncryptedZip'
      ]([], 6, 'test-password');
    ...
    it('does not call registerFormat twice on repeated encrypted zip calls', async () => {
      ...
      const createEncryptedZip = (
        runner as unknown as Record<string, (...args: unknown[]) => Promise<unknown>>
      ...
      expect(registerSpy.mock.calls.filter((c) => c[0] === 'zip-encrypted').length).toBeLessThanOrEqual(1);
